...
class EnergyReportingTests(unittest.TestCase):
    ...
    def test_rotor_energy_totals_include_reducer_corrected_energy(self):
        totals = rotor_energy_totals(
            rotor_positive_energy=[1.0, 2.0],
            rotor_negative_energy=[0.5, 1.5],
            reducer_corrected_energy=[1.8, 3.2],
        )

        self.assertAlmostEqual(totals["reducer_corrected_energy_8"], 5.0)
