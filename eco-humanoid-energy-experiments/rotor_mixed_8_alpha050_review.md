...
注意：`E_rotor` 列是固定评估里的 8 电机转子端机械能单位距离指标，用于横向比较真实转子能耗；`E_joint` 列是固定评估里的 10 关节侧机械能单位距离指标，用于检查旧 ECO 口径。最后一列的 cost 是否通过，使用各方法自己的训练分布 cost 定义；因此 `legacy_joint_abs_10` 的 `5397.6021 < 5540.5193` 是旧 ECO 训练 cost，不是前面的转子端 `E_rotor/d_x`。

| 方法 | cost 定义 | 阈值 | 固定成功率 | 速度 | $E_{rotor,pos}/d_x$ | $E_{rotor,neg}/d_x$ | $E_{joint,pos}/d_x$ | $E_{joint,neg}/d_x$ | 训练分布成功率 | 摔倒数 | 成功回合 cost 是否通过 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| baseline `model_3000` | 原始策略 | - | 20/20 | 0.1166 | 8.0737 | 8.3499 | 10.4327 | 10.8831 | 89% | 11 | - |
| `rotor_positive_8` seed0 | $P^+$ | 30.8848 | 20/20 | 0.1198 | 7.3911 | 7.8181 | 9.9027 | 10.4252 | 92% | 8 | 否，30.9197 > 30.8848 |
| `rotor_abs_8` seed0 | $P^+ + P^-$ | 50.8560 | 20/20 | 0.1216 | 7.4331 | 7.7111 | 9.7019 | 10.1154 | 86% | 14 | 否，51.5796 > 50.8560 |
| `rotor_mixed_8_alpha050` seed0 | $P^+ + 0.5P^-$ | 40.8955 | 20/20 | 0.1184 | 7.4248 | 7.8655 | 9.9094 | 10.4597 | 88% | 12 | 否，41.6067 > 40.8955 |
| `legacy_joint_abs_10` seed0 | 旧 ECO，10 关节 $|\tau\dot q|$ | 5540.5193 | 20/20 | 0.1129 | 7.4846 | 7.8795 | 9.3527 | 9.9502 | 88% | 12 | 是，5397.6021 < 5540.5193 |
