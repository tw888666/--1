...
本表只使用 fixed-command 20 回合评估的 `episode_summary.csv`。`cost 定义` 表示训练时施加的约束目标；后面的 `E_rotor`、`E_yaw` 和 `E_joint` 不是训练 cost，而是对每个训练后策略统一重算的一组评估指标。因此不同方法都会有 $E_{rotor,pos}/d_x$、$E_{rotor,neg}/d_x$、$E_{yaw,pos}/d_x$、$E_{yaw,neg}/d_x$、$E_{joint,pos}/d_x$ 和 $E_{joint,neg}/d_x$，这样才能在同一把尺子下横向比较。`E_yaw` 只统计两个 hip yaw 关节；`E_joint` 统计 10 个关节。`20回合总 d_x` 是 20 回合 `distance_x` 的总和；`distance_x` 是单回合平均行走距离；`平均速度` 是 `mean_velocity_x` 的回合平均。所有单位距离指标均按 `sum(energy) / sum(distance_x)` 计算。
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 原始策略参考 `model_3000` | 原始未继续训练策略 | 20/20 | 55.9929 | 2.7996 | 0.1166 | 8.0737 | 8.3499 | 0.1948 | 0.3690 | 10.4327 | 10.8831 |
| `legacy_joint_abs_10` seed0（旧 ECO 基线） | 旧 ECO，10 关节 $\lvert\tau\dot q\rvert$ | 20/20 | 54.2354 | 2.7118 | 0.1129 | 7.4846 | 7.8795 | 0.1581 | 0.3607 | 9.3527 | 9.9502 |
| `rotor_positive_8` seed0 | $P^+$ | 20/20 | 57.5218 | 2.8761 | 0.1198 | 7.3911 | 7.8181 | 0.2625 | 0.3580 | 9.9027 | 10.4252 |
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 原始策略参考 `model_3000` | 原始未继续训练策略 | 20/20 | 55.9929 | 2.7996 | 0.1166 | 8.0737 | 8.3499 | 0.1948 | 0.3690 | 10.4327 | 10.8831 |
| `legacy_joint_abs_10` seed0-5 均值（旧 ECO 基线） | 旧 ECO，10 关节 $\lvert\tau\dot q\rvert$ | 120/120 | 54.5474 | 2.7274 | 0.1136 | 7.5220 | 7.8459 | 0.1607 | 0.3583 | 9.2804 | 9.8019 |
| `rotor_positive_8` seed0-5 均值 | $P^+$ | 120/120 | 57.4537 | 2.8727 | 0.1196 | 7.6344 | 8.0524 | 0.2554 | 0.3810 | 10.1725 | 10.7160 |
| `rotor_abs_8` seed0-5 均值 | $P^+ + P^-$ | 120/120 | 56.1303 | 2.8065 | 0.1169 | 7.5157 | 7.8413 | 0.2082 | 0.3585 | 9.9648 | 10.4407 |
| `joint_positive_8` seed0-5 均值 | 8 驱动关节 $P^+$ | 120/120 | 55.1245 | 2.7562 | 0.1148 | 7.6088 | 8.0452 | 0.2437 | 0.3746 | 9.7225 | 10.2898 |

#### 六种子负功来源表

本表对应上面的六种子固定评估 20 回合表。除原始策略参考 `model_3000` 仍为单次参考外，其他方法均为 seed0-5：每个 seed 先计算自己的单位距离转子负功、相位负功占比、主要电机/关节负功占比和 yaw 占比，再对 6 个 seed 做算术平均。电机名含义同上：`hip_motor` 是髋部传动电机，`lower_motor` 是下肢传动电机，括号中保留原始 CSV 字段缩写。

| 方法 | $E_{rotor,neg}/d_x$ | 双支撑 | 左支撑 | 右支撑 | 过渡/腾空 | 转子负功主要电机（占转子负功） | 10 关节负功主要关节（占 10 关节负功） | yaw 负功占 10 关节负功 |
| --- | ---: | ---: | ---: | ---: | ---: | --- | --- | ---: |
| 原始策略参考 `model_3000` | 8.3499 | 25.4% | 35.9% | 37.5% | 1.2% | 右下肢传动电机0 (R lower m0) 28.3%<br>左下肢传动电机0 (L lower m0) 23.3%<br>右髋部传动电机0 (R hip m0) 16.0% | 右膝俯仰关节 (R knee) 31.1%<br>左膝俯仰关节 (L knee) 26.6%<br>右髋横滚关节 (R hip roll) 12.5% | 3.4% |
| `legacy_joint_abs_10` seed0-5 均值（旧 ECO 基线） | 7.8459 | 28.2% | 35.5% | 34.9% | 1.3% | 右下肢传动电机0 (R lower m0) 25.6%<br>左下肢传动电机0 (L lower m0) 22.4%<br>右髋部传动电机0 (R hip m0) 17.0% | 右膝俯仰关节 (R knee) 27.9%<br>左膝俯仰关节 (L knee) 26.1%<br>右髋横滚关节 (R hip roll) 12.1% | 3.7% |
| `rotor_positive_8` seed0-5 均值 | 8.0524 | 22.4% | 38.2% | 38.1% | 1.2% | 右下肢传动电机0 (R lower m0) 30.5%<br>左下肢传动电机0 (L lower m0) 27.4%<br>右髋部传动电机0 (R hip m0) 13.7% | 右膝俯仰关节 (R knee) 32.4%<br>左膝俯仰关节 (L knee) 30.1%<br>右髋横滚关节 (R hip roll) 10.0% | 3.6% |
| `rotor_abs_8` seed0-5 均值 | 7.8413 | 24.2% | 38.1% | 36.4% | 1.3% | 右下肢传动电机0 (R lower m0) 27.0%<br>左下肢传动电机0 (L lower m0) 26.5%<br>右髋部传动电机0 (R hip m0) 14.4% | 左膝俯仰关节 (L knee) 29.3%<br>右膝俯仰关节 (R knee) 28.8%<br>右髋横滚关节 (R hip roll) 10.5% | 3.4% |
| `joint_positive_8` seed0-5 均值 | 8.0452 | 26.5% | 34.7% | 37.4% | 1.3% | 右下肢传动电机0 (R lower m0) 29.7%<br>左下肢传动电机0 (L lower m0) 24.3%<br>右髋部传动电机0 (R hip m0) 14.9% | 右膝俯仰关节 (R knee) 32.2%<br>左膝俯仰关节 (L knee) 27.7%<br>右髋横滚关节 (R hip roll) 11.0% | 3.6% |

六种子口径下结论和 seed0 基本一致：转子负功主要来自左右支撑相，下肢传动电机0 和膝俯仰关节是最主要来源；yaw 关节负功只占 10 关节负功约 `3.4%` 至 `3.7%`，不是负功总量的主要解释。
